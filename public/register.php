                ...
                <div class="auth-card">
                    ...
                    <?php if (isset($_GET['status']) && $_GET['status'] === 'pending_approval'): ?>
                        <div class="alert alert-info">
                            ...
                            Hesabınız oluşturuldu ve admin onayı bekliyor. Onaylandıktan sonra giriş yapabilirsiniz.
                        </div>
                    <?php endif; ?>
                    ...
                    <div class="auth-forms-container">
                        ...
                        <!-- Register Form -->
                        <form action="<?php echo htmlspecialchars($_SERVER['REQUEST_SCHEME'] . '://' . $_SERVER['HTTP_HOST'] . '/src/actions/handle_register.php'); ?>" 
                                method="POST" 
                                id="registerForm" 
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-note">Geçerli bir e-posta adresi girin. Onay bilgileri bu adrese gönderilecektir.</div>
                            </div>
                            ...
                            <div class="password-requirements">
                                <span class="password-requirements-title">Şifre Gereksinimleri:</span>
                                <ul>
                                    <li>En az 6 karakter uzunluğunda olmalıdır</li>
                                    <li>Güvenliğiniz için büyük-küçük harf, rakam ve özel karakter kullanın</li>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function validateForm(form) {
        ...
        console.log('Validating form:', form.id, 'with inputs:', inputs.length); // Debug
        
        inputs.forEach(input => {
            const value = input.value.trim();
            
            console.log('Validating input:', input.name, 'value:', value); // Debug
            ...
            if (!value) {
                ...
                isValid = false;
                console.log('Input failed: empty value -', input.name); // Debug
                return;
            }
            ...
            switch(input.type) {
                case 'email':
                    ...
                    if (!emailRegex.test(value)) {
                        input.classList.add('error');
                        isValid = false;
                        console.log('Email validation failed:', value); // Debug
                    } else {
                ...
                case 'password':
                    if (value.length < 6) {
                        input.classList.add('error');
                        isValid = false;
                        console.log('Password too short:', value.length); // Debug
                    } else {
                        input.classList.add('success');
                    }
                    break;
                ...
                default:
                    if (input.name === 'username' && form.id === 'registerForm') {
                        ...
                        if (!usernameRegex.test(value)) {
                            input.classList.add('error');
                            isValid = false;
                            console.log('Username validation failed:', value); // Debug
                        } else {
                            input.classList.add('success');
                        }
                    } else {
        ...
        if (form.id === 'registerForm') {
            const regPassword = form.querySelector('#reg_password');
            const regConfirmPassword = form.querySelector('#reg_confirm_password');
            ...
            if (regPassword && regConfirmPassword && regPassword.value && regConfirmPassword.value) {
                if (regPassword.value !== regConfirmPassword.value) {
                    ...
                    isValid = false;
                    console.log('Password confirmation failed'); // Debug
    ...
    // Password strength indicator (register form only)
    ...
    // Password strength checking (register form)
    if (passwordInput) {
    ...
    // Username input filter (register form)
    if (usernameInput) {
    ...
    // Form submissions - COMPLETELY SIMPLIFIED
    document.addEventListener('click', function(e) {
        if (e.target && e.target.id === 'registerBtn') {
            ...
            const ingame_name = document.getElementById('reg_ingame_name').value.trim();
            ...
            console.log('Form values:', {username, email, ingame_name, password, confirm_password, csrf_token});
            ...
            console.log('Submitting form with action:', submitForm.action);
            submitForm.submit();
        }
    ...
    // Backup: Remove all complex form event listeners
    ...
    if (registerFormElement) {
        registerFormElement.addEventListener('submit', function(e) {
            e.preventDefault();
            // Do nothing - let click handler handle it
        });
    }
