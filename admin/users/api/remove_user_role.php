<?php
// /admin/users/api/remove_user_role.php
...
try {
    ...
    if (!$is_super_admin && !can_user_manage_user_roles($pdo, $user_id, $_SESSION['user_id'])) {
        http_response_code(403);
        echo json_encode(['success' => false, 'message' => 'Bu kullanıcının rollerini yönetme yetkiniz yok']);
        exit;
    }
    ...
    if (!$is_super_admin && !can_user_manage_role($pdo, $role_id, $_SESSION['user_id'])) {
        http_response_code(403);
        echo json_encode(['success' => false, 'message' => 'Bu rolü kaldırma yetkiniz yok']);
        exit;
    }
    
    // Rol adını al
    $role_check = $pdo->prepare("SELECT name FROM roles WHERE id = :role_id");
    ...
    if ($existing_check->fetch()['count'] == 0) {
        http_response_code(400);
        echo json_encode(['success' => false, 'message' => 'Kullanıcının bu rolü zaten yok']);
        exit;
    }
    ...
    echo json_encode([
        'success' => true,
        'message' => "'{$role['name']}' rolü başarıyla kaldırıldı"
    ]);
    ...
    error_log("Remove user role general error: " . $e->getMessage());
    http_response_code(500);
    echo json_encode([
        'success' => false,
