...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
                ...
                case 'reject_application':
                    ...
                    case 'kick_member':
                        ...
                        try {
                            ...
                            ");
                            $stmt->execute([$team_id, $member['user_id']]);
                            ...
                            $pdo->commit();
                            $success_message = $member['username'] . ' takımdan atıldı ve tekrar başvurabilir.';
                            
                        } catch (Exception $e) {
                            $pdo->rollBack();
                            throw new Exception('Üye atılırken bir hata oluştu: ' . $e->getMessage());
                        }
                        break;
        ...
        ");
                    ...
                    if (!$application) {
                        throw new Exception('Başvuru bulunamadı veya işlenebilir durumda değil.');
                    }

                    // Yetki kontrolü
                    if (!($is_team_owner || $is_team_manager || has_team_permission($pdo, $team_id, $current_user_id, 'team.manage.applications'))) {
                    ...
                    if (review_team_application($pdo, $application_id, $review_action, $current_user_id, $notes)) {
                        ...
                        throw new Exception('Başvuru işlenirken bir hata oluştu.');
                    }
                    break;

                case 'bulk_action':
                    // Toplu başvuru işlemleri
                    $bulk_action_type = $_POST['bulk_action_type'] ?? '';
                    $selected_applications = isset($_POST['selected_applications']) && is_array($_POST['selected_applications'])
                        ? array_filter($_POST['selected_applications'], 'is_numeric')
                        : [];

                    if (!in_array($bulk_action_type, ['approve_all', 'reject_all'])) {
                        throw new Exception('Geçersiz toplu işlem türü.');
                    }

                    if (empty($selected_applications)) {
                        throw new Exception('Hiçbir başvuru seçilmedi.');
                    }
                    ...
                    $success_count = 0;
                    $review_action = $bulk_action_type === 'approve_all' ? 'approved' : 'rejected';
            ...
            ");
                        ...
                        if ($stmt->fetch()) {
                            if (review_team_application($pdo, $app_id, $review_action, $current_user_id, 'Toplu işlem')) {
                                $success_count++;
                            }
                        }
                    }
                    ...
                    if ($success_count > 0) {
                        $action_text = $bulk_action_type === 'approve_all' ? 'onaylandı' : 'reddedildi';
                        $success_message = $success_count . ' başvuru ' . $action_text . '.';
                    } else {
                        throw new Exception('Hiçbir başvuru işlenemedi.');
                    }
                    break;
...
<div class="site-container">
    ...
    <div class="team-manage-container">
        ...
        <div class="manage-header">
            <div class="header-info">
                <div class="team-info">
                    <?php if ($team_data['logo_path']): ?>
                        <img src="/uploads/teams/logos/<?= htmlspecialchars($team_data['logo_path']) ?>"
                            alt="<?= htmlspecialchars($team_data['name']) ?>" class="team-logo-small"
                            style="border-color: <?= htmlspecialchars($team_data['color']) ?>;">
                    <?php else: ?>
                        <div class="team-logo-small"
                            style="border-color: <?= htmlspecialchars($team_data['color']) ?>; color: <?= htmlspecialchars($team_data['color']) ?>;">
                ...
                <div class="quick-stats">
                    <div class="stat-card">
                        <div class="stat-value"><?= $team_stats['total_members'] ?>/<?= $team_data['max_members'] ?>
                        </div>
        ...
        <div class="manage-tabs">
            <nav class="tab-nav">
                <a href="?id=<?= $team_id ?>&tab=overview"
                    class="tab-link <?= $active_tab === 'overview' ? 'active' : '' ?>">
                    <i class="fas fa-chart-line"></i> Genel Bakış
                </a>
                <a href="?id=<?= $team_id ?>&tab=members"
                    class="tab-link <?= $active_tab === 'members' ? 'active' : '' ?>">
                    <i class="fas fa-users"></i> Üyeler (<?= $team_stats['total_members'] ?>)
                </a>
                <a href="?id=<?= $team_id ?>&tab=applications"
                    class="tab-link <?= $active_tab === 'applications' ? 'active' : '' ?>">
                ...
                <?php if ($is_team_owner): ?>
                    ...
                    <a href="?id=<?= $team_id ?>&tab=settings"
                        class="tab-link <?= $active_tab === 'settings' ? 'active' : '' ?>">
