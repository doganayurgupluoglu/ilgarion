...
function audit_user_login($pdo, $user_id, $username, $login_method = 'standard') {
    add_audit_log($pdo, 'Kullanıcı giriş yaptı', 'user', $user_id, null, [
        'username' => $username,
...
function audit_user_logout($pdo, $user_id, $username) {
    add_audit_log($pdo, 'Kullanıcı çıkış yaptı', 'user', $user_id, null, [
        'username' => $username,
        'timestamp' => date('Y-m-d H:i:s')
    ], $user_id);
}
...
function audit_role_created($pdo, $role_id, $role_data, $user_id = null) {
    add_audit_log($pdo, 'Yeni rol oluşturdu', 'role', $role_id, null, $role_data, $user_id);
}
...
function audit_role_updated($pdo, $role_id, $old_data, $new_data, $user_id = null) {
    add_audit_log($pdo, 'Rol bilgilerini güncelledi', 'role', $role_id, $old_data, $new_data, $user_id);
}
...
function audit_role_deleted($pdo, $role_id, $role_data, $user_id = null) {
    add_audit_log($pdo, 'Rol sildi', 'role', $role_id, $role_data, null, $user_id);
}
...
function audit_role_permissions_updated($pdo, $role_id, $old_permissions, $new_permissions, $user_id = null) {
    add_audit_log($pdo, 'Rol yetkilerini güncelledi', 'role', $role_id, [
...
function audit_role_viewed($pdo, $role_id, $role_name = null, $user_id = null) {
    add_audit_log($pdo, 'Rol detaylarını görüntüledi', 'role', $role_id, null, [
        'role_name' => $role_name,
...
function audit_user_created($pdo, $created_user_id, $user_data, $creator_user_id = null) {
    ...
    add_audit_log($pdo, 'Yeni kullanıcı oluşturdu', 'user', $created_user_id, null, $safe_data, $creator_user_id);
}
...
function audit_user_updated($pdo, $updated_user_id, $old_data, $new_data, $updater_user_id = null) {
    ...
    add_audit_log($pdo, 'Kullanıcı bilgilerini güncelledi', 'user', $updated_user_id, $safe_old_data, $safe_new_data, $updater_user_id);
}
...
function audit_user_deleted($pdo, $deleted_user_id, $user_data, $deleter_user_id = null) {
    ...
    add_audit_log($pdo, 'Kullanıcı sildi', 'user', $deleted_user_id, $safe_data, null, $deleter_user_id);
}
...
function audit_user_role_assigned($pdo, $user_id, $role_id, $role_name, $assigner_user_id = null) {
    add_audit_log($pdo, 'Kullanıcıya rol atadı', 'user', $user_id, null, [
        'role_id' => $role_id,
        'role_name' => $role_name,
...
function audit_user_role_removed($pdo, $user_id, $role_id, $role_name, $remover_user_id = null) {
    add_audit_log($pdo, 'Kullanıcıdan rol kaldırdı', 'user', $user_id, [
        'role_id' => $role_id,
        'role_name' => $role_name,
...
function audit_page_accessed($pdo, $page_name, $additional_data = null, $user_id = null) {
    ...
    add_audit_log($pdo, 'Sayfa erişimi: ' . $page_name, 'page', null, null, $audit_data, $user_id);
}
...
function audit_settings_changed($pdo, $setting_key, $old_value, $new_value, $user_id = null) {
    add_audit_log($pdo, 'Sistem ayarı değiştirdi', 'setting', null, [
        'key' => $setting_key,
        'value' => $old_value
    ], [
        'key' => $setting_key,
...
function audit_permission_created($pdo, $permission_id, $permission_data, $user_id = null) {
    add_audit_log($pdo, 'Yeni yetki oluşturdu', 'permission', $permission_id, null, $permission_data, $user_id);
}
...
function audit_permission_updated($pdo, $permission_id, $old_data, $new_data, $user_id = null) {
    add_audit_log($pdo, 'Yetki bilgilerini güncelledi', 'permission', $permission_id, $old_data, $new_data, $user_id);
}
...
function audit_permission_deleted($pdo, $permission_id, $permission_data, $user_id = null) {
    add_audit_log($pdo, 'Yetki sildi', 'permission', $permission_id, $permission_data, null, $user_id);
}
...
function audit_security_event($pdo, $event_type, $event_data, $user_id = null) {
    add_audit_log($pdo, 'Güvenlik olayı: ' . $event_type, 'security', null, null, $event_data, $user_id);
}
...
function audit_failed_login($pdo, $username, $reason = 'invalid_credentials') {
    add_audit_log($pdo, 'Başarısız giriş denemesi', 'security', null, null, [
        'username' => $username,
        'reason' => $reason,
        'timestamp' => date('Y-m-d H:i:s')
    ], null);
...
function audit_password_changed($pdo, $user_id, $username, $changed_by_admin = false, $changer_user_id = null) {
    add_audit_log($pdo, 'Şifre değiştirdi', 'user', $user_id, null, [
        'username' => $username,
...
function audit_email_changed($pdo, $user_id, $old_email, $new_email, $user_id_changer = null) {
    add_audit_log($pdo, 'E-posta adresini değiştirdi', 'user', $user_id, [
        'email' => $old_email
    ], [
        'email' => $new_email
...
function audit_user_status_changed($pdo, $user_id, $username, $old_status, $new_status, $changer_user_id = null) {
    add_audit_log($pdo, 'Kullanıcı durumunu değiştirdi', 'user', $user_id, [
        'username' => $username,
        'status' => $old_status
    ], [
        'username' => $username,
...
function audit_data_exported($pdo, $export_type, $export_params, $user_id = null) {
    add_audit_log($pdo, 'Veri dışa aktardı: ' . $export_type, 'export', null, null, [
...
function audit_file_uploaded($pdo, $file_path, $file_info, $user_id = null) {
    add_audit_log($pdo, 'Dosya yükledi', 'file', null, null, [
        'file_path' => $file_path,
        'file_info' => $file_info,
        'timestamp' => date('Y-m-d H:i:s')
    ], $user_id);
...
function audit_file_deleted($pdo, $file_path, $file_info, $user_id = null) {
    add_audit_log($pdo, 'Dosya sildi', 'file', null, [
        'file_path' => $file_path,
        'file_info' => $file_info
    ], null, $user_id);
}
...
function get_security_events($pdo, $limit = 50, $days = 7) {
    try {
        $query = "
            ...
            FROM audit_log al
            LEFT JOIN users u ON al.user_id = u.id
            WHERE (al.action LIKE '%güvenlik%' 
            OR al.action LIKE '%başarısız%'
            OR al.action LIKE '%yetkisiz%'
            OR al.action LIKE '%geçersiz%')
...
function cleanup_old_audit_logs($pdo, $days_to_keep = 365) {
    try {
        ...
        add_audit_log($pdo, 'Audit log temizliği yapıldı', 'system', null, null, [
