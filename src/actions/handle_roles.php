...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    try {
        ...
        if ($action === 'create_role' || $action === 'update_role') {
            ...
            // Temel rolleri silmeyi engelle - SADECE ADMIN KORUNACAK
            if (!is_role_deletable($role_to_delete['name'])) {
                $_SESSION['error_message'] = "Korumalı roller güvenlik nedeniyle silinemez. (Sadece 'admin' rolü korunuyor)";

                // Audit log - yetkisiz silme girişimi
                audit_log($pdo, $current_user_id, 'unauthorized_role_delete_attempt', 'role', $role_id_to_delete, null, [
                    'role_name' => $role_to_delete['name'],
                    'reason' => 'Protected role deletion attempt - only admin protected'
                ]);
            ...
            // Süper admin korumalı rollerin kontrolü - SADECE ADMIN
            $super_admin_protected_roles = ['admin']; // member, dis_uye kaldırıldı
